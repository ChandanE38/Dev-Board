...
export default function LoginPage() {
  ...
  const [form, setForm] = useState({ githubUsername: '', password: '' });
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10">
      ...
      <div className="relative w-full max-w-md rounded-[2rem] border border-white/10 bg-white/[0.05] p-8 shadow-glow backdrop-blur-2xl">
        ...
        <h2 className="mt-3 text-3xl font-semibold text-slate-100">Welcome back</h2>
        <p className="mt-2 text-sm text-slate-400">Sign in to continue to your dashboard.</p>

        <form onSubmit={submit} className="mt-8 space-y-4">
          <input
            className="premium-focus w-full rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 outline-none ring-0 placeholder:text-slate-500 transition focus:border-cyan-400/50"
